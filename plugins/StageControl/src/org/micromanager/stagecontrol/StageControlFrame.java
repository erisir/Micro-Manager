...
public class StageControlFrame extends javax.swing.JFrame {
	...
	private void setRelativeStagePosition(double z)
	{
		try {
			...
			if (!core_.deviceBusy(device)){
				double from = core_.getPosition(device);
				core_.setRelativePosition(device, z);
				double target = core_.getPosition(device);
				double delta = target - from;
				msg.setText(String.format("ZPos:  to-from = %.3f-%.3f=%.3f(¦Ìm)",target,from,delta));
			}
		} catch (Exception e) {
			gui_.logError(e);
		}
