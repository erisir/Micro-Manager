...
public class FRETStartDetector {
	...
	private static FRETStartDetector instance;
	...
	public static FRETStartDetector getInstance()
	{
		if(instance == null)
			instance  = new FRETStartDetector();
		return instance;
	}
	...
	private byte[] readAnswer()
	{
		...
		try {
			...
			String answerold = new String(readBuffer1);
			if(!answer.equals(answerold)){
				LogMessage(answer);
			}
			if(answer.substring(0,9).equals("FRETSTART")){
				synchronized(MMT.Acqlock){
					MMT.FRETStartListenerRun=false;
				}
				MMT.isFRETStart=true;
	...
	public void listenStart(){
		(new Thread(new Runnable() {
			@Override
			public void run() {
				while(MMT.FRETStartListenerRun){
						readAnswer();
				}
			}
		})).start();
